...
// ─── Розміщення по колу ───────────────────────────────────
const TABLE_SIZE = 1250   // px (квадрат контейнера)
const SEAT_RADIUS = 490   // відстань від центру до картки
const TABLE_RADIUS = 200   // радіус круглого стола
...
function getSeatPos(index: number, total: number) {
  if (total === 10) {
    ...
    const maxPositions = [
      { x: 200, y: 130 },  // Громадянин 1 (Зверху-ліворуч)
      { x: 625, y: 130 },  // Громадянин 2 (Зверху-центр)
      { x: 1050, y: 130 }, // Громадянин 3 (Зверху-праворуч)
      { x: 1090, y: 440 }, // Громадянин 4 (Справа-вгорі)
      { x: 1090, y: 810 }, // Громадянин 5 (Справа-внизу)
      { x: 1050, y: 1120 },// Громадянин 6 (Знизу-праворуч)
      { x: 625, y: 1120 }, // Громадянин 7 (Знизу-центр)
      { x: 200, y: 1120 }, // Громадянин 8 (Знизу-ліворуч)
      { x: 160, y: 810 },  // Громадянин 9 (Зліва-внизу)
      { x: 160, y: 440 }   // Громадянин 10 (Зліва-вгорі)
    ]
    return maxPositions[index] || { x: 625, y: 625 }
  }

  // Коли кількість менша за максимум: довільна форма у вигляді ракушки (спіралі)
  // Радіус плавно збільшується від 380px до 470px, створюючи витончений спіральний ефект
  const startAngle = -Math.PI / 2
  const angle = startAngle + (2 * Math.PI * index) / total
  const spiralRadius = 380 + (index / (total || 1)) * 90
