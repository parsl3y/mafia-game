...
// ─── Розміщення по колу ───────────────────────────────────
const TABLE_SIZE = 1150   // px (квадрат контейнера)
const SEAT_RADIUS = 460   // відстань від центру до картки
...
function getSeatPos(index: number, total: number) {
  if (total === 10) {
    ...
    // Координати симетрично розташовані навколо центральної точки (575, 575)
    const maxPositions = [
      { x: 245, y: 150 }, // Зверху-зліва
      { x: 575, y: 150 }, // Зверху-посередині
      { x: 905, y: 150 }, // Зверху-справа
      { x: 1000, y: 375 }, // Справа-вгорі
      { x: 1000, y: 775 }, // Справа-внизу
      { x: 905, y: 1000 }, // Знизу-справа
      { x: 575, y: 1000 }, // Знизу-посередині
      { x: 245, y: 1000 }, // Знизу-зліва
      { x: 150, y: 775 }, // Зліва-внизу
      { x: 150, y: 375 }  // Зліва-вгорі
    ]
    return maxPositions[index] || { x: 575, y: 575 }
  }

  // Коли кількість менша за максимум: довільна форма у вигляді ракушки (спіралі)
  // Радіус плавно збільшується від 390px до 480px, створюючи витончений спіральний ефект
  const startAngle = -Math.PI / 2
  const angle = startAngle + (2 * Math.PI * index) / total
  const spiralRadius = 390 + (index / (total || 1)) * 90
